...
import { insertCustomMarker } from '@/lib/docxRaportTemplate'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { error: upErr } = await sb.storage.from('documente').upload(path, templateBuffer, { contentType: sablon.fisier_tip })
    if (upErr) return NextResponse.json({ error: upErr.message }, { status: 500 })

    const { error: sablonErr } = await sb.from('proiect_documente').update({
      fisier_path: path, fisier_marime: templateBuffer.length, updated_at: new Date().toISOString(),
    }).eq('id', sablon.id)
    if (sablonErr) {
