...
export async function GET(req: NextRequest) {
  ...
  const { data: doc } = await sb.from('inbox_watch_files').select('fisier_path,fisier_nume,fisier_tip').eq('id', id).single()
  if (!doc) return NextResponse.json({ error: 'Fișierul nu a fost găsit' }, { status: 404 })
