...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!url || !firmaId || !lunaId)
      return NextResponse.json({ error: 'Completează linkul Oblio, firma și luna' }, { status: 400 })

    let source: URL
    try { source = new URL(url) } catch { return NextResponse.json({ error: 'Link Oblio invalid' }, { status: 400 }) }
    if (source.protocol !== 'https:' || !source.hostname.endsWith('oblio.eu'))
      return NextResponse.json({ error: 'Sunt acceptate doar linkuri HTTPS Oblio' }, { status: 400 })
    ...
    if (!(response.headers.get('content-type') || '').includes('pdf'))
      return NextResponse.json({ error: 'Linkul Oblio nu a returnat un PDF' }, { status: 422 })
