...
export async function GET(req: NextRequest) {
  ...
  const { data: doc } = await sb.from('model_documente').select('fisier_path,fisier_nume,fisier_tip').eq('id', id).single()
  ...
  return new NextResponse(Buffer.from(await file.arrayBuffer()), { headers: safeHeaders(doc.fisier_nume, doc.fisier_tip || file.type) })
}
